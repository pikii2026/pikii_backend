...
| **ContactEmail** | VARCHAR           | -            | 연락용 이메일 (**이력서 생성 시점**에 가입 이메일을 복사해 초기값으로 넣고, 이후 사용자가 변경 가능) |
...
| 입력 항목        | 저장 위치                              |
|:------------ |:---------------------------------- |
| 연락용 이메일 (자동) | `MemberResume.ContactEmail`        |
...
이력서를 **생성하는 시점**에 회원의 가입 이메일(`Member.Email`)을 `ContactEmail`로 복사한다.

- 회원가입 시점이 아니라 **이력서 생성 시점**에 복사한다. (이력서가 없으면 저장할 곳도 없다)
- 이후 사용자가 연락용 이메일을 별도로 변경할 수 있다.
- 이력서는 전체 공개이므로 `ContactEmail`도 다른 회원에게 노출된다.
