...
| **AutoFilled** | BOOLEAN, DEFAULT FALSE | -    | 조율 개설 시 캘린더 겹침으로 시스템이 자동 생성한 응답인지 (팀원이 직접 제출하면 false로 바뀐다) |
...
- **조율 개설(7-10) 시점에, 참가 팀원 전원의 개인 캘린더(`MemberSchedule`)를 조율 기간 내에서 전개하여 겹치는 슬롯은 해당 팀원의 응답을 `Available=false, AutoFilled=true`로 미리 생성한다.** (반복 일정은 전개하여 판정) 이는 응답 제출로 취급하지 않으며 `MeetingPollMember.Responded`는 그대로 false다.
- 겹치지 않는 슬롯은 응답 row 자체를 만들지 않는다 (row 없음 = 기본 '가능').
- 팀원이 7-12에서 직접 제출하면 해당 팀원의 모든 슬롯에 대해 응답이 갱신되고 `AutoFilled=false`로 바뀐다 (이제 자동 추정이 아니라 본인이 확정한 값이므로).
- 슬롯별 `가능 인원 = Available = true 인 응답 수` — 자동 생성된 '불가' 응답도 집계에 포함된다.
- **미응답자는 가능/불가 어느 쪽으로도 집계하지 않으며, 미응답 인원으로 별도 표시한다.** (단, 캘린더로 자동 생성된 '불가' 응답은 위 집계 규칙대로 이미 응답 row가 존재하므로 미응답으로 잡히지 않는다)
