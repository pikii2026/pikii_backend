...
- 탐색 기간 × 탐색 시간대를 **30분 단위**로 나누어 슬롯 후보를 생성한다.
- **참가 팀원 중 한 명이라도 개인 캘린더(`MemberSchedule`)에 겹치는 일정이 있는 슬롯은 개설(7-10) 시점에 제외**하고 저장한다 (반복 일정은 전개하여 판정). 개설 이후 새로 등록한 캘린더 일정은 반영되지 않는다.
- 제외되고 남은 슬롯은 **When2meet 방식**으로 전체를 격자(그리드)로 노출한다. 이 이상으로는 후보를 잘라내지 않는다.
