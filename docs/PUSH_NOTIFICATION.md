...
이 문서는 구현 전 설계/계획 문서였다. **2026-08-07 구현 완료** — 최신 스펙은 API_SPEC.md(9-8/9-9), DB_Schema.md(DeviceToken), ENUM.md(Platform, 22번) 참고. 실제 기기로의 푸시 수신 확인만 아직 안 됨(모바일 앱 쪽에서 실 토큰 등록 필요). 이하 원본 설계 내용은 구현 근거/이력으로 남겨둔다.
